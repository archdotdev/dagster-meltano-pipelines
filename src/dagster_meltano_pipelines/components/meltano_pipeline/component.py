    ...
    with tempfile.TemporaryDirectory(prefix="meltano_ssh_") as temp_dir:
        ...
        with contextlib.ExitStack() as stack:
            ...
            for i, key_content in enumerate(ssh_private_keys):
                # Resolve SSH private key from environment variable
                resolved_key_content: t.Optional[str]
                if isinstance(key_content, dg.EnvVar):
                    resolved_key_content = key_content.get_value()
                    if not resolved_key_content:
                        continue
                else:
                    resolved_key_content = key_content

                # Replace literal \n with actual newlines
                resolved_key_content = resolved_key_content.replace("\\n", "\n")
                if not resolved_key_content.endswith("\n"):
                    resolved_key_content += "\n"

                key_file = stack.enter_context(
                    tempfile.NamedTemporaryFile(mode="w", suffix=f"_id_rsa_{i}", dir=temp_dir, delete=False)
                )
                key_file.write(resolved_key_content)
                key_file.flush()
...
class MeltanoPipeline(dg.PermissiveConfig):
    """Pipeline definition."""
    ...
    git_ssh_private_keys: t.List[t.Union[str, dg.EnvVar]] = Field(
...
class DagsterAssetProps(dg.PermissiveConfig):
